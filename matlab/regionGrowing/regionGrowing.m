...
disp(['RegionGrowing Opening: Initial position (' num2str(initPos(1))...
      ...
      num2str(cIM(initPos(1),initPos(2),:)) ' as initial pixel value!'])
...
%init1 = cIM(initPos(1), initPos(2), 1);
%init2 = cIM(initPos(1), initPos(2), 2);
%init3 = cIM(initPos(1), initPos(2), 3);
    ...
    for j = -1:1
        ...
        if xv+i > 0  &&  xv+i <= nRow &&...          % within the x-bounds?
           ...
           sqrt( (xv+i-initPos(1))^2 +...
            ...
            r1 = cIM(xv+i, yv+j, 1);
            r2 = cIM(initPos(1), initPos(2), 1);
            g1 = cIM(xv+i, yv+j, 2);
            g2 = cIM(initPos(1), initPos(2), 2);
            b1 = cIM(xv+i, yv+j, 3);
            b2 = cIM(initPos(1), initPos(2), 3);

            dest1 = power(double(r1) - double(r2), 2);
            dest2 = power(double(g1) - double(g2), 2);
            dest3 = power(double(b1) - double(b2), 2);
            dest4 = dest1+dest2+dest3;
            similaridade = sqrt(dest4);

            %dest = [dest1 dest2 dest3 dest4 similaridade]           
            
            if similaridade <= thresVal  % range of the threshold?
